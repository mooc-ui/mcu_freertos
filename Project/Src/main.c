...
//  CLK_Struct.PCLK.Divider   = 1;  //原值为2 
...
void Clock_Init(void)
{
  ...
  CLK_Struct.HCLK.Divider   = 1;    //FLASH运行程序最高支持26MHz，所以/2  //20210630 modify for test
  CLK_Struct.PCLK.Divider   = 2;    //原值为1 20220104 wwb modify 1 -> 2  
  CLK_ClockConfig(&CLK_Struct);
}
...
void uarts_init(void)
{
  ...
  /* UART0 RX pin(IOA12), input mode 显示屏通信串口*/
  ...
  /* UART1 RX pin(IOA13), input mode 充电底座通信串口*/
  GPIO_InitStruct.GPIO_Mode = GPIO_Mode_INPUT; //GPIO_Mode_INPUT
...
void pwm0_init(void)
{
  ...
  PWM_OutputCmd(PWM0, PWM_CHANNEL_0, ENABLE);  //pwm3无波形输出
...
void main_motor_pwm_init(void)
{
  ...
  /** PWM0 channel 1 Initialize:
       ...
       High/Low = (0x8000-0x2000)/(0xFFFF-0x6000) = 3/5
    */
  //PWM_OLineConfig(PWM3_OUT0, PWM_OLINE_0); //原来写的是0，20220204 wwb modify
  PWM_OLineConfig(PWM3_OUT1, PWM_OLINE_1);	
  PWM_OCInitStruct.OutMode = PWM_OUTMOD_RESET_SET; //PWM_OUTMOD_TOGGLE_RESET
  //PWM_OCInitStruct.Period = 256;     //频率8K	
...
//  PWM_OC1Init(PWM0, &PWM_OCInitStruct);   //！！！！！PWM_OC1Init(PWM0, &PWM_OCInitStruct);->特别注意不要写错了，不要写成:PWM_OC0Init
//	
  ...
  PWM_OC2Init(PWM0, &PWM_OCInitStruct);   //！！！！！PWM_OC1Init(PWM0, &PWM_OCInitStruct);->特别注意不要写错了，不要写成:PWM_OC0Init
...
//32.768MHz理论主频,PCLK不分频，最小只能支持到30Hz的pwm波
...
void PMU_init(void)
{
  ...
  //PMU_PDNDSleepConfig(PMU_VDCINPDNS_1, PMU_VDDPDNS_1);     //要去掉
  ...
  LowPower_InitStruct.VDCINDetector       = PMU_VDCINDET_DISABLE;
	LowPower_InitStruct.VDDDetector         = PMU_VDDDET_DISABLE;   //新增的
...
void adc_auto_trig_init(void)
{
  ...
  TMR_InitStruct.Period = 13107 - 1;  //原值13107200 - 1;  1ms触发一次adc采样
...
void GPIO_init(void)
{
	...
	UART_DeInit(UART0);	   //IOB2将设为io口	
//	//IOB6 水泵控制；IOB12 充电使能 IOB15 红外发送；水泵改为pwm1控制
//	GPIO_InitStruct.GPIO_Pin = GPIO_Pin_6 | GPIO_Pin_12 | GPIO_Pin_15;
//  GPIOBToF_Init(GPIOB, &GPIO_InitStruct);
//	GPIOBToF_ResetBits(GPIOB, GPIO_InitStruct.GPIO_Pin);
	//IOB12 充电使能 IOB15 红外发送；
	GPIO_InitStruct.GPIO_Pin = GPIO_Pin_12 | GPIO_Pin_15;
  GPIOBToF_Init(GPIOB, &GPIO_InitStruct);
	GPIOBToF_ResetBits(GPIOB, GPIO_InitStruct.GPIO_Pin);	

	//IOD12 电解水12V控制;IOD14 wifi模组进入AT指令模式(不使用时上拉);IOD15 12V_EN 暂时用12V_EN来使能显示屏的3.3V
  ...
  GPIOBToF_Init(GPIOF, &GPIO_InitStruct);
	...
	//IOA3电机转速检测(用于电机转速检测) IOA5模式键,IOA6消毒键,IOA7直立判断 //IOA9 换刷检测
	GPIO_InitStruct.GPIO_Mode = GPIO_Mode_INPUT;
...
void brush_err_notify(void)
{
		...
		{
				if (0 == sys_time.brush_err_notify_cnt) //滚刷类型错误，间隔5s语音播报
				{}                              
...
void bottom_unconnect_notify(void)
{
		...
		{
				if (0 == sys_time.bottom_unconnect_notify_cnt) //滚刷类型错误，间隔5s语音播报
				{}                              
...
void wsx_full_notify(void)
{
		...
		{
				if (0 == sys_time.wsx_full_notify_cnt) //污水箱满，间隔5s语音播报
				{}                              
...
{
		if(0 == GPIOA_ReadInputDataBit(GPIOA, GPIO_Pin_7))  //直立识别，低电平有效
		{
			if (++sys_time.upright_check_cnt >= 3)  //延时去抖
			{
...
{
		if(1 == GPIOBToF_ReadInputDataBit(GPIOF, GPIO_Pin_0))  //安装检测
		{
			if (++sys_time.wsx_uninstall_check_cnt >= 3)  //延时去抖
			{
		...
		switch(work_mode.mode)
		{
			case INTELLIGENT_MODE:        //智能模式
			  ...
			  //3、对应电机开启使能
				...
				{
					if (sys_status.val)            //直立或连接底座，停所有电机
			...
			case AUTO_CLEAN_MODE:        //自清洁模式
				...
				if (++work_mode.query_timeout_cnt >= 100)  //1s查询一次	
					work_mode.query_timeout_cnt = 0;
		
			  bottom_unconnect_notify();
				if (!sys_status.flags.bottom_connected)    //底座未连接，退出
					break;
			
				switch(work_mode.step)
				{
					case 2:              //自清洗,所有电机工作,主电机更高速运转
						for (i = 0; i < MOTOR_CTRL_NUM; i++)
						{
							if (sys_status.val)            //直立或连接底座，开所有电机
								motor_ctrl[i].en = ENABLE;
							else                           //关所有电机
								motor_ctrl[i].en = DISABLE;
						}
						if (work_mode.time_cnt >= 1200)   //2分钟清洗结束
						{
								work_mode.time_cnt = 0;
								work_mode.step = 1;
								motor_ctrl[MAIN_MOTOR].en = ENABLE;   //关主电机
							  motor_ctrl[PUMP_MOTOR].en = DISABLE;  //关水泵
							
								uart1_send_cmd(DRY_START);            //1、给底座发送开始烘干开始指令
						}
						
						break;
					
					case 3:              //烘干
						if (sys_status.val)                    //直立底座连接时，滚刷一直保持运行
							motor_ctrl[BRUSH_MOTOR].en = ENABLE;
						else                          
							motor_ctrl[BRUSH_MOTOR].en = DISABLE;
						
						if (work_mode.time_cnt >= 180000)     //烘干30分钟
						{
								motor_ctrl[BRUSH_MOTOR].en = DISABLE;
								work_mode.step = 0;
...
{   
	if (++uart[2].tx_interval >= 5)  //500ms 查询一次电池信息
	{
...
void testPwm(void)
{
  ...
  PWM_OC1Init(PWM3, &PWM_OCInitStruct);
	...
	PWM3->CCR0 = 1024;                 //由ClockSource和ClockDivision一起决定   频率8K
...
int main(void)
{
  ...
  PMU_WakeUpPinConfig(GPIO_Pin_5, IOA_FALLING);
	PMU_ClearIOAINTStatus(GPIO_Pin_5);//按键2
	PMU_WakeUpPinConfig(GPIO_Pin_6, IOA_FALLING);
	PMU_ClearIOAINTStatus(GPIO_Pin_6);//按键3
	
	PMU_WakeUpPinConfig(GPIO_Pin_7, IOA_FALLING);
	PMU_ClearIOAINTStatus(GPIO_Pin_7);//直立识别	

	PMU_WakeUpPinConfig(GPIO_Pin_3, IOA_FALLING);
	PMU_ClearIOAINTStatus(GPIO_Pin_3);//电机转速测量	
